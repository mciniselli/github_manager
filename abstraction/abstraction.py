...
class Abstraction:
    def __init__(self, java_file):
        '''
        This class allows you to abstract java file and the tokens. We do not use CommandLine class to improve performances (this operation has to be
        done lots of time
        e.g.
        a=Abstraction(file.java)
        a.abstract_method()
    ...
    def abstract_method(self):
        '''
        this function calls src2abs to abstract the current java file
        '''
        try:
    ...
    def read_file_txt(self, file_path):
        '''
        this function reads the file @file_path using codecs module. it returns a list of all lines
        '''
        file = None
        try:
    ...
    def write_tokens(self, element):  # write generic file
        '''
        this function write the tokens into tokens.txt. We decided to pass a str(List) object to this function
        We use the abstracted method to generate the list of raw tokens.
        We implemented another function to retrieve the list of all tokens (token_extraction.py) that does not
        require the abstraction (since the abstraction is very slow!)
        '''
        file = None
        try:
    ...
    def process_map(self, filename):  # write map file in a better way
        '''
        this function allows you to read the map file generated by src2abs during abstraction
        a dict object (more readable) is returned
        '''
        lines = list()
    ...
    def save_list_of_tokens(self):
        '''
        this function read the abstract file and, using the mapping file, is able to retrieve the list
        of raw tokens
        For example, if we have the abstract public void METHOD_1, we use mapping to retrieve the raw name
        of METHOD_1 (e.g. print). Then we can save the following list of tokens:
        ["public", "void", "print"]
        NB: during abstraction some tokens are automatically removed (e.g. annotations)
        '''
        try:
...
class AbstractionManager:
    def __init__(self, min_tokens: int = 0, max_tokens: int = 9999999, min_lines: int = 0, max_lines: int = 9999999):
        '''
        this function is the manager of @Abstraction class. It is able to process all repos mined and absract them
        we can pass the minimum and maximum number of tokens and lines that the method has to fulfill in order
        to be abstracted.
        e.g.
        am=AbstractionManager(0, 200, 0, 15)
        # methods with a 0<=num_tokens<=200 and 0<=num_lines<=15 lines have to be processed
        am.abstract_mined_repos()
        '''
            ...
            for file_id in file_ids:
                ...
                for method_id, num_tokens, num_lines, nested, already_required, is_abs_ok in zip(method_ids,
                    ...
                    if already_required == 'True':
                        # this file has already been required during previous abstractions, so we can skip it
    ...
    def update_method(self, method_dict, method_path, method_field, abstraction_result, method_to_abstract):
        '''
        update method_info.csv file with information about the abstraction result
        '''
